...
with open('config.yaml',encoding='utf-8') as f:
    ...
    PATH_PLAYER = str(res['config_path']['player']).strip()
    PATH_SHOP = str(res['config_path']['shop']).strip()
    PATH_CCULOG = str(res['config_path']['ccuLog']).strip()
    PATH_BAT = str(res['config_path']['batDir']).strip()
    FILE_BAT = str(res['config_path']['batFile']).strip()
